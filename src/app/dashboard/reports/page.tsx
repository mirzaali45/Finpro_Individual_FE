...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        applyFilters(invoicesData, dateRange, clientFilter, statusFilter, sortField, sortDirection);
      } catch (error) {
  ...
  const applyFilters = (
    ...
    // Sort the data
    filtered.sort((a, b) => {
      let aValue: any = a[sort as keyof Invoice];
      let bValue: any = b[sort as keyof Invoice];
      
      // Handle date comparison
      if (sort === 'issue_date' || sort === 'due_date' || sort === 'created_at') {
        aValue = new Date(aValue).getTime();
        bValue = new Date(bValue).getTime();
      }
      
      // Handle numeric comparison
      else if (sort === 'total_amount' || sort === 'subtotal' || sort === 'tax_amount') {
  ...
  const handleFilterChange = () => {
    applyFilters(invoices, dateRange, clientFilter, statusFilter, sortField, sortDirection);
  };
  ...
  const handleSortChange = (field: string) => {
    ...
    applyFilters(invoices, dateRange, clientFilter, statusFilter, field, newDirection);
  };
  ...
  const exportToCSV = () => {
    ...
    // Create CSV content
    const headers = ['Invoice #', 'Date', 'Due Date', 'Client', 'Status', 'Amount', 'Tax', 'Total'];
                  ...
                  return (
                    <tr key={invoice.invoice_id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-primary">
                        <a href={`/dashboard/invoices/${invoice.invoice_id}`} className="hover:underline">
                          {invoice.invoice_number}
                        </a>
                      </td>

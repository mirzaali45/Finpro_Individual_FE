...
export default function ProductDetailPage({
  ...
  const [isDeleted, setIsDeleted] = useState(false); // New state to track if product has been deleted

  useEffect(() => {
    const fetchProductData = async () => {
      // Skip fetching if product has been deleted
      if (isDeleted) return;
  ...
  const handleDeleteProduct = async () => {
    try {
      ...
      // Set the deleted flag to prevent further API calls
      setIsDeleted(true);
  ...
  const handleArchiveProduct = async () => {
    try {
      ...
      console.log(`Attempting to archive product ${product.product_id}`);
      ...
      console.log("Product archived successfully:", updatedProduct);
      ...
      setError(`Failed to archive product: ${errorMessage}`);
      toast({
        title: "Error",
  ...
  const handleRestoreProduct = async () => {
    try {
      ...
      console.log(`Attempting to restore product ${product.product_id}`);
      ...
      console.log("Product restored successfully:", updatedProduct);
      ...
      setError(`Failed to restore product: ${errorMessage}`);
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  // If we've deleted the product, show nothing (will redirect)
  if (isDeleted) {
    return null;
  }
